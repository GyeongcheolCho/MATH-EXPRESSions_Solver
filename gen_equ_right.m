function [List_equ,List_score] = gen_equ_right(x,Vec_Answer_c,N_Sides,Boxes_Index,Mat_Nrange,List_equ,List_score)
    Mat_Nrange(1,1)=1;
    value_ch=eval(char(Vec_Answer_c(1,1:(x-1)))); % solve the equation and save it to value_ch in the char format
    ...
    if (leng_value==N_Sides(1,2)) &&... % the number of digits equals to the number of blank on the right side 
        ...
        List_score=[List_score;sum(ind_score,1)-Penalty-sum((Mat_Nrange(:,1)<Mat_Nrange(:,2))+(Mat_Nrange(:,1)>Mat_Nrange(:,3)),1)];
    end
end